...
pub async fn analyze(addr: &str, verbose: bool, peers: NetworkPeers) -> Result<()> {
    ...
    match analysis {
        ...
        Err(AnalysisError::UnrecognizedInput) => {
            ...
            println_if_verbose!("Provided string was not recognized as a data address, trying other types...");
...
fn try_other_types(addr: &str, verbose: bool) {
    ...
    if let Some(data_map) = try_private_address {
        ...
        println_if_verbose!("💡 This local address points to a DataMap which is stored on your local machine.");
        println_if_verbose!("💡 Using this DataMap you can download your Private Archive from the Network.");
